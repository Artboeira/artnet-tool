...
const TAB_IDS: ReadonlySet<string> = new Set<TabId>(['cue-pad', 'monitor', 'settings']);

export function AppShell() {
  const [activeTab, setActiveTab] = useState<TabId>('cue-pad');

  return (
    <div className="w-full flex flex-col h-screen bg-background text-foreground overflow-hidden">
      <Tabs
        value={activeTab}
        onValueChange={(v) => { if (TAB_IDS.has(v)) setActiveTab(v as TabId); }}
        className="flex flex-col flex-1 overflow-hidden"
      >
        <TabsList aria-label="Main navigation" className="w-full justify-start shrink-0 rounded-none bg-background border-b border-border px-2">
          <TabsTrigger value="cue-pad">Cue Pad</TabsTrigger>
          <TabsTrigger value="monitor">Monitor</TabsTrigger>
          <TabsTrigger value="settings">Settings</TabsTrigger>
        </TabsList>

        <TabsContent value="cue-pad" className="flex-1 overflow-auto p-4 mt-0">
          {/* TODO (Story 3.x): Replace with CuePad feature component */}
          <p className="text-muted-foreground">Cue Pad — coming in Epic 3</p>
        </TabsContent>

        <TabsContent value="monitor" className="flex-1 overflow-auto p-4 mt-0">
